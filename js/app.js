...
function renderQuestion() {
  ...
  item.order.forEach((optIdx, pos) => {
    ...
    // Native <button>: activated by click / Enter / Space / the number key.
    // Not role="radio" — the app has no arrow-key nav between options (arrows
    // move between questions), so radiogroup semantics would misdescribe it.
    btn.setAttribute('aria-keyshortcuts', String(pos + 1));
  ...
  if (isAnswered && run.mode === 'exam') {
    ...
    [...el.options.children].forEach((b, i) => {
      b.disabled = true;
      if (i === item.chosen) {
        b.classList.add('option--picked');
        b.insertAdjacentHTML('beforeend', '<span class="visually-hidden"> (נבחר)</span>');
      }
    });
    hide(el.feedback);
  } else if (isAnswered) {
...
function applyAnswered(item, silent) {
  ...
  buttons.forEach((b, i) => {
    b.disabled = true;
    const status = [];
    if (i === item.chosen) status.push('נבחר');
    if (i === item.correctPos) {
      b.classList.add('is-correct');
      status.push('התשובה הנכונה');
    ...
    // the ✓/✗ marks are decorative (aria-hidden); convey the same state to
    // assistive tech in text, since color alone isn't accessible.
    if (status.length) {
      b.insertAdjacentHTML('beforeend',
        `<span class="visually-hidden"> (${status.join(', ')})</span>`);
    }
  });
...
document.addEventListener('DOMContentLoaded', boot);  examTimerToggle: $('#exam-timer-toggle'),
  examMinutes: $('#exam-minutes'),
  examCount: $('#exam-count'),
  srsToggle: $('#srs-toggle'),
  setupSummary: $('#setup-summary'),
